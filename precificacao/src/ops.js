...
(function (global) {
  ...
  function stepper(etapa, compacto) {
    const idx = ETAPAS.findIndex(e => e.id === etapa);
    return `<div class="stepper ${compacto ? "mini" : ""}">${ETAPAS.map((e, i) => `
      <div class="step ${i < idx ? "done" : ""} ${i === idx ? "now" : ""}">
        <span class="dot">${i < idx ? "✓" : i + 1}</span>
        ${compacto ? "" : `<span class="lbl">${e.nome}</span>`}
  ...
  function renderLista() {
    ...
    lista.innerHTML = rows.map(i => {
      ...
      const resp = i.responsavel_email ? esc(i.responsavel_email.split("@")[0]) : "<i>sem responsável</i>";
  ...
  async function abrirDetalhe(id) {
    ...
    body.innerHTML = `
      ...
      ${stepper(impl.etapa, false)}
      <div class="ops-meta" style="margin:10px 0 16px">
        <span>👤 ${impl.responsavel_email ? esc(impl.responsavel_email) : "<i>sem responsável</i>"}</span>
        <span>📅 início ${dataBR(impl.inicio)}</span>
